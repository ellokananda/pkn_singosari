...
<?php
	...
	$sql = $koneksi->query("SELECT count(Nik) as lokal from daftar_pegawai");
...
?>
 ...
 <?php
	$sql = $koneksi->query("SELECT count(nip) as putra from daftar_pegawai where LENGTH(nip) >= 12");
	while ($data= $sql->fetch_assoc()) {
		$putra=$data['putra'];
	}
?> 
...
<?php
	$sql = $koneksi->query("SELECT count(nip) as putri from daftar_pegawai where LENGTH(nip) < 12");
	while ($data= $sql->fetch_assoc()) {
...
<?php
	$sql = $koneksi->query("SELECT count(Id_Pengguna) as boyong from login");
	while ($data= $sql->fetch_assoc()) {
...
<div class="row">
	...
	<div class="col-lg-3 col-6">
		...
		<div class="small-box bg-success">
			...
			<a href="index.php?page=data-pegawai" class="small-box-footer">Informasi
			</a>
		</div>
	</div>
	<!-- ./col -->
